...
export const forexFoundationsQuizV1: AssessmentDefinition = {
  ...
  governance: {
    ...
    reviewer: "Milestone 13 content review",
    reviewedAt: "2026-09-17",
    nextReviewAt: "2027-03-17",
    sources: [
      "PipStart Forex Foundations lessons",
      "WEBSITE FX 1 roadmap sections 22.4 and Milestone 13",
    ],
  },
  questions: [
    {
      ...
      choices: [
        {
          ...
          label: "Exchanging one currency for another",
        },
    ...
    },
    {
      ...
      explanation:
        "The first currency in a pair is the base currency, so EUR is the base in EUR/USD.",
      type: "single-choice",
      choices: [
        { id: "eur", label: "EUR" },
        { id: "usd", label: "USD" },
    ...
    },
    {
      ...
      explanation:
        "A pip describes a small price movement, while a lot describes trade size.",
      type: "multiple-answer",
      choices: [
        {
          id: "pip-price-move",
          label: "A pip measures a small price movement",
        },
        { id: "lot-trade-size", label: "A lot describes the size of a trade" },
        {
          id: "pip-market-hours",
          label: "A pip tells you when a market session opens",
        },
    ...
    },
    {
      ...
      explanation:
        "The spread is the gap between the price at which the market buys and the price at which it sells.",
      type: "true-false",
      choices: [
        { id: "true", label: "True" },
    ...
    },
    {
      ...
      explanation:
        "Different financial centres are active at different times, which changes market activity and liquidity.",
      type: "single-choice",
      choices: [
        {
          id: "activity-varies",
        ...
        {
          id: "fixed-daily-price",
          label: "Each session fixes currency prices for the day",
        },
        {
          id: "weekend-only",
          label: "Forex can only be traded during weekend sessions",
        },
      ],
    ...
    },
    {
      ...
      explanation:
        "Forex participants include banks, businesses and individual traders.",
      type: "multiple-answer",
      choices: [
...
function validateQuestion(question: AssessmentQuestion) {
  ...
  if (
    !["single-choice", "multiple-answer", "true-false"].includes(question.type)
  ) {
  ...
  if (
    new Set(question.correctChoiceIds).size !== question.correctChoiceIds.length
  ) {
    throw new Error(`Duplicate correct choice id in "${question.id}"`);
  }
  for (const id of question.correctChoiceIds) {
    if (!choiceIds.has(id))
      throw new Error(`Unknown correct choice "${id}" in "${question.id}"`);
  }
  ...
  if (
    ...
    question.correctChoiceIds.length < 2
  ) {
    throw new Error(
      `Multiple-answer question "${question.id}" needs at least two correct choices`,
    );
  }
  if (
    ...
    throw new Error(
      `True-false question "${question.id}" must use true and false choices`,
    );
  }
}
...
function validateCurriculumReference(assessment: AssessmentDefinition) {
  if (!getLearningPath(assessment.learningPath)) {
    throw new Error(
      `Unknown published learning path "${assessment.learningPath}"`,
    );
  }
  ...
  if (!course)
    throw new Error(`Unknown published course "${assessment.courseId}"`);
  ...
  if (!assessment.moduleId)
    throw new Error(`${assessment.scope} assessment requires moduleId`);
  ...
  if (
    ...
    throw new Error(
      `Unknown module "${assessment.moduleId}" for course "${assessment.courseId}"`,
    );
  }

  if (assessment.scope === "module") {
    if (assessment.lessonId)
      throw new Error("Module assessment cannot declare lessonId");
    return;
  }

  if (!assessment.lessonId)
    throw new Error("Lesson assessment requires lessonId");
  if (
    !curriculumModule.lessons.some(
      (lesson) => lesson.type === "lesson" && lesson.id === assessment.lessonId,
    )
  ) {
    throw new Error(
      `Unknown published lesson "${assessment.lessonId}" for module "${assessment.moduleId}"`,
    );
  }
}
    ...
    for (const level of path.levels)
      for (const course of level.courses) {
        ...
        for (const curriculumModule of course.modules) {
          for (const requirement of curriculumModule.assessmentRequirements ??
            [])
            validateRequirement(requirement, registry, {
              courseId: course.id,
...
export function assertValidAssessmentRegistry(
  ...
  for (const assessment of registry) {
    ...
    nonEmpty(
      assessment.governance.author,
      `Assessment "${assessment.id}" author`,
    );
    nonEmpty(
      assessment.governance.reviewer,
      `Assessment "${assessment.id}" reviewer`,
    );
    if (!assessment.governance.sources.length) {
      throw new Error(
        `Assessment "${assessment.id}" requires at least one source`,
      );
    }
    for (const source of assessment.governance.sources) {
      nonEmpty(source, `Assessment "${assessment.id}" source`);
    }
    const reviewedAt = Date.parse(assessment.governance.reviewedAt);
    const nextReviewAt = Date.parse(assessment.governance.nextReviewAt);
    if (!Number.isFinite(reviewedAt) || !Number.isFinite(nextReviewAt)) {
      throw new Error(`Assessment "${assessment.id}" has invalid review dates`);
    }
    if (nextReviewAt <= reviewedAt) {
      throw new Error(
        `Assessment "${assessment.id}" next review must follow its review date`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Assessment "${assessment.id}" has invalid passing percentage`,
      );
    }
    if (
      !Number.isInteger(assessment.retakeCooldownSeconds) ||
      assessment.retakeCooldownSeconds < 0 ||
      assessment.retakeCooldownSeconds > 3600
    ) {
      throw new Error(
        `Assessment "${assessment.id}" has invalid retake cooldown`,
      );
    }
    ...
    if (versions.has(key))
      throw new Error(`Duplicate assessment version "${key}"`);
    versions.add(key);
    ...
    for (const question of assessment.questions) {
      if (questionIds.has(question.id)) {
        throw new Error(
          `Duplicate question id "${question.id}" in "${assessment.id}"`,
        );
      }
    ...
    if (assessment.status === "published")
      validateCurriculumReference(assessment);
  }
...
export function getCurrentPublishedAssessment(id: string) {
  return assessmentRegistry
    .filter(
      (assessment) => assessment.id === id && assessment.status === "published",
    )
    .sort((left, right) => right.version - left.version)[0];
}

export function getContinuableAssessment(id: string, version: number) {
  const assessment = getAssessment(id, version);
  return assessment && ["published", "retired"].includes(assessment.status)
    ? assessment
    : undefined;
}
